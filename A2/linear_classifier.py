...
def svm_loss_vectorized(
    ...
    scores = X @ W  # (N, C)
    correct_scores = scores[torch.arange(N), y]  # (N,)
    margins = scores - correct_scores[:, None] + 1  # (N, C)
    margins[torch.arange(N), y] = 0  # Correct class margin is zero
    ...
    mask = margins > 0  # (N, C)
    counts_per_example = torch.sum(mask, dim=1)  # (N,)
    mask = mask.to(torch.double)
    mask[torch.arange(N), y] = -counts_per_example.to(mask.dtype)
    dW = X.T @ mask
    dW /= N
    dW += 2 * reg * W
    #############################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    # 计算每个样本在每个类别的得分
    scores = X @ W  # (N, C)，N 是样本数，C 是类别数
    # 选择得分最高的类别作为预测标签
    y_pred = torch.argmax(scores, dim=1)  # 返回每行最大得分的索引
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [3e-3, 4e-3, 5e-3]
    regularization_strengths = [3e-2, 4e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    # 训练模型，注意我们传递了权重 W 作为训练的参数
    W, loss = train_linear_classifier(svm_loss_vectorized, None, X_train, y_train, lr, reg, num_iters)

    # 计算训练集的准确率
    train_pred = predict_linear_classifier(W, X_train)
    train_acc = (train_pred == y_train).double().mean().item()

    # 计算验证集的准确率
    val_pred = predict_linear_classifier(W, X_val)
    val_acc = (val_pred == y_val).double().mean().item()
...
def softmax_loss_vectorized(
    ...
    # 数值稳定性：每行减去最大值
    scores -= torch.max(scores, dim=1, keepdim=True).values  # 保持维度一致
    # 计算 Softmax 概率
    exp_scores = torch.exp(scores)  # (N, C)
    probs = exp_scores / torch.sum(exp_scores, dim=1, keepdim=True)  # (N, C)
    # 计算损失
    correct_log_probs = -torch.log(probs[torch.arange(N), y])  # 提取正确类的概率
    loss = torch.sum(correct_log_probs) / N  # 平均损失
    loss += reg * torch.sum(W * W)  # 加入正则化项
    # 计算梯度
    dscores = probs.clone()  # (N, C)
    dscores[torch.arange(N), y] -= 1  # 正确类的概率减 1
    dW = X.T @ dscores / N  # (D, N) @ (N, C) -> (D, C)
    dW += 2 * reg * W  # 加入正则化梯度
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [5.1e-3, 5.2e-3, 5.3e-3, 5.4e-3, 5.5e-3]
    regularization_strengths = [1e-5, 2e-5, 5e-5, 7e-5, 1e-4]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
