...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        # Apply padding with correct device
          ...
          for f in range(F):  # Iterate over filters
              for i in range(H_out):
                  for j in range(W_out):
                      # Extract the corresponding region from the padded input
                      h_start = i * stride
                      w_start = j * stride
    ...
    def backward(dout, cache):
        ...
        x_padded = torch.nn.functional.pad(x, (pad, pad, pad, pad), mode='constant', value=0)
        dx_padded = torch.zeros_like(x_padded)  # To store dx before removing padding
        for f in range(F):
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        # Update dx (backpropagation through the filter)
                        dx_padded[n, :, h_start:h_start+HH, w_start:w_start+WW] += dout[n, f, i, j] * w[f]

        dx = dx_padded[:, :, pad:H+pad, pad:W+pad]  # Crop to match the original shape
        ###############################################################
        #                       END OF YOUR CODE                      #
        ###############################################################
        return dx, dw, db
...
class MaxPool(object):
            ...
            for c in range(C):  # Loop over channels
                for i in range(H_out):  # Loop over height
                    for j in range(W_out):  # Loop over width
                        ...
                        # Extract pooling region and apply max
    ...
    def backward(dout, cache):
        ...
        dx = torch.zeros_like(x, dtype=dout.dtype)  # Match dtype to avoid precision issues
            ...
            for c in range(C):  
                for i in range(H_out):  
                    for j in range(W_out):  
                        ...
                        # Use torch.isclose instead of ==
                        max_mask = torch.isclose(x_pool, torch.max(x_pool))

                        # Distribute gradient
                        dx[n, c, h_start:h_end, w_start:w_end] += dout[n, c, i, j] * max_mask / max_mask.sum()
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims=input_dims, num_classes=10,
                        ...
                        max_pools=[0, 1],
                    weight_scale=7e-2,
                    reg=7e-5, dtype=torch.float, device= "cuda")
    ...
    solver = Solver(model, data_dict,
                    ...
                    lr_decay = 0.95,
                    print_every=100000, print_acc_every=5,
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        dbeta = dout.sum(dim=0)  # Sum over batch (N,)
        dgamma = torch.sum(dout * x_normalized, dim=0)  # Element-wise multiplication with normalized input
        # Compute dx
        dx_normalized = dout * gamma  # Multiply by gamma
        dvar = torch.sum(dx_normalized * (x - mean) * -0.5 * (var + eps) ** (-1.5), dim=0)  # Variance gradient
        dmean = torch.sum(dx_normalized * -1 / torch.sqrt(var + eps), dim=0) + dvar * torch.sum(-2 * (x - mean), dim=0) / N  # Mean gradient
        dx = dx_normalized / torch.sqrt(var + eps) + dvar * 2 * (x - mean) / N + dmean / N  # Final dx
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        # Compute dx using simplified formula
        dx = (1. / N) * gamma * (var + eps) ** -0.5 * (
            N * dout - dout.sum(dim=0) - x_normalized * torch.sum(dout * x_normalized, dim=0)
        )
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Apply batch normalization (using the fully connected version)
    ...
    def backward(dout, cache):
        ...
        # Compute batch normalization backward
