...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        nn.init.constant_(self.pred_cls.bias, 0)  # 后续会覆盖
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            # 分类分支：先经过 stem_cls，再通过 pred_cls 得到分类 logits
            cls_feat = self.stem_cls(feat)
            cls_pred = self.pred_cls(cls_feat)  # shape: (B, num_classes, H, W)
            
            # 框回归和 centerness分支：共用 stem_box
            box_ctr_feat = self.stem_box(feat)
            box_pred = self.pred_box(box_ctr_feat)   # shape: (B, 4, H, W)
            ctr_pred = self.pred_ctr(box_ctr_feat)     # shape: (B, 1, H, W)
            
            # 将每个输出从 (B, C, H, W) 转换为 (B, H*W, C)（即先将空间维度展平，并将通道置后）
            B, _, H, W = cls_pred.shape
            cls_pred = cls_pred.permute(0, 2, 3, 1).reshape(B, H * W, -1)
            box_pred = box_pred.permute(0, 2, 3, 1).reshape(B, H * W, -1)
            ctr_pred = ctr_pred.permute(0, 2, 3, 1).reshape(B, H * W, -1)
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    # Ensure we use only the first four coordinates even if a class label is present.
    boxes = gt_boxes[:, :4]
    
    # Compute deltas
    # left: distance from location x to box's left edge
    left = locations[:, 0] - boxes[:, 0]
    # top: distance from location y to box's top edge
    top = locations[:, 1] - boxes[:, 1]
    # right: distance from box's right edge to location x
    right = boxes[:, 2] - locations[:, 0]
    # bottom: distance from box's bottom edge to location y
    bottom = boxes[:, 3] - locations[:, 1]
    
    # Stack the deltas into a tensor of shape (N, 4)
    deltas = torch.stack([left, top, right, bottom], dim=1)
    
    # Normalize by the stride
    deltas = deltas / stride
    
    # Identify background boxes: rows where all box coordinates are -1
    background_mask = (boxes == -1).all(dim=1)
    
    # For background boxes, set deltas to (-1, -1, -1, -1)
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Un-normalize the deltas by multiplying with the stride.
    deltas = deltas * stride
    
    # Clip negative values to zero to ensure the center is inside the box.
    deltas = torch.clamp(deltas, min=0.0)
    
    # Split deltas into respective components.
    left = deltas[:, 0]
    top = deltas[:, 1]
    right = deltas[:, 2]
    bottom = deltas[:, 3]
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = None
    # Replace "pass" statement with your code
    # Identify background boxes where all delta values are -1.
    background_mask = (deltas == -1).all(dim=1)
    
    # Unpack the L, T, R, B values.
    left, top, right, bottom = deltas.unbind(dim=1)
    
    # To avoid division by zero, we clamp the denominator with a small epsilon.
    eps = 1e-6
    horizontal = torch.min(left, right) / (torch.max(left, right).clamp(min=eps))
    vertical = torch.min(top, bottom) / (torch.max(top, bottom).clamp(min=eps))
    
    # Compute centerness as the geometric mean of horizontal and vertical ratios.
    centerness = torch.sqrt(horizontal * vertical)
    
    # For background boxes, override centerness with -1.
...
class FCOS(nn.Module):
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1: For each box, select the most confident class and its score.
            # Replace "pass" statement with your code
            max_pred_scores, most_confident_classes = level_pred_scores.max(dim=1)

            # Step 2: Only retain predictions with scores above the threshold.
            # Replace "pass" statement with your code
            ...
            # Retrieve the stride for the current FPN level.
            # (Assuming you have stored these in self.fpn_strides)
            # Replace "pass" statement with your code
