...
def create_sample_tensor() -> Tensor:
    ...
    x = torch.tensor([[0, 10], [100, 0], [0, 0]], dtype=torch.float32)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    # Extracting the third column (index 2, so we select all rows and just the third column)
    third_col = x[:, 2:3]
    
    # Extracting the first two rows and the first three columns
    first_two_rows_three_cols = x[:2, :3]
    
    # Extracting the even rows and odd columns
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    # Replace "pass" statement with your code
    C = 1 + max(x)
    # Convert x to a tensor of type int64 for indexing
    x_tensor = torch.tensor(x, dtype=torch.int64)
    # Create a tensor of one-hot vectors
    y = torch.eye(C)[x_tensor]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    y = torch.cat((x_reshaped[0], x_reshaped[3], 
            x_reshaped[1], x_reshaped[4], 
            x_reshaped[2], x_reshaped[5]), dim=0)
    y=y.view(3, 8)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    # Initialize the result tensor with the proper shape
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Step 1: Calculate the column means (mu_j)
    mu = x.sum(dim=0) / x.size(0)  # Sum of each column, then divide by number of rows

    # Step 2: Calculate the column standard deviations (sigma_j)
    # Variance is the mean of squared differences from the mean
    variance = ((x - mu)**2).sum(dim=0) / (x.size(0)-1)
    sigma = torch.sqrt(variance)  # Standard deviation is the square root of variance

    # Step 3: Normalize each element (x[i, j] - mu_j) / sigma_j
    y = (x - mu) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
