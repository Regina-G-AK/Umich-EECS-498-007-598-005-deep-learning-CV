...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flat = x_test.view(num_test, -1)
    # 规范化
    x_train_norms = (x_train_flat ** 2).sum(dim=1)  # Shape: (num_train,)
    x_test_norms = (x_test_flat ** 2).sum(dim=1)    # Shape: (num_test,)
    # 点乘
    dot_product = x_train_flat @ x_test_flat.T       # Shape: (num_train, num_test)
    # 计算欧式距离
    dists = x_train_norms[:, None] + x_test_norms[None, :] - 2 * dot_product
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        # Get distances from the j-th test sample to all training samples
        distances = dists[:, j]
        # 找到最小距离
        ...
        # Find the label with the highest frequency (smallest label in case of a tie)
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        for j in range(num_test):
            ...
            # Find the label with the highest frequency (smallest label in case of a tie)
            y_test_pred[j] = torch.argmax(counts)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    for fold in range(num_folds):
        ...
        # Iterate through each k in k_choices
        for k in k_choices:
            # Initialize the KnnClassifier (assume it's already implemented elsewhere)
            classifier = KnnClassifier(x_train_fold, y_train_fold)

            # Predict on the validation fold
            y_pred = classifier.predict(x_val_fold, k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    mean_accuracies = {k: sum(acc) / len(acc) for k, acc in k_to_accuracies.items()}
    # Find the k with the highest mean accuracy
    best_k = min(
        (k for k, mean_acc in mean_accuracies.items() if mean_acc == max(mean_accuracies.values())),
        key=lambda k: k
    )
    ##########################################################################
    #                           END OF YOUR CODE                             #
