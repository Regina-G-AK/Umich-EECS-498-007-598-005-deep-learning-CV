...
def gram_matrix(features, normalize=True):
    ...
    # Compute Gram matrix: (N, C, H*W) x (N, H*W, C) -> (N, C, C)
...
def tv_loss(img, tv_weight):
    ...
    # Replace "pass" statement with your code
    loss = 0.0
    # Differences between adjacent pixels in vertical direction (height)
    vert_diff = img[:, :, 1:, :] - img[:, :, :-1, :]
    # Differences between adjacent pixels in horizontal direction (width)
    horiz_diff = img[:, :, :, 1:] - img[:, :, :, :-1]
    
    loss = tv_weight * (torch.sum(vert_diff ** 2) + torch.sum(horiz_diff ** 2))
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  for r in range(R):
    # Get features and mask for region r
    feat_r = features[:, r]     # (N, C, H, W)
    mask_r = masks[:, r].unsqueeze(1)  # (N, 1, H, W) for broadcasting

    # Apply mask to features
    masked_feat = feat_r * mask_r  # (N, C, H, W)

    # Reshape to (N, C, H*W)
    masked_feat = masked_feat.view(N, C, -1)

    # Compute Gram matrix (N, C, C)
    gram_r = torch.bmm(masked_feat, masked_feat.transpose(1, 2))

    if normalize:
      gram_r = gram_r / (C * H * W)

    guided_gram_list.append(gram_r)

  # Stack over R dimension: list of (N, C, C) -> (N, R, C, C)
  guided_gram = torch.stack(guided_gram_list, dim=1)
  return guided_gram
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    loss = torch.tensor(0.0, device=feats[0].device, dtype=feats[0].dtype)

    for i in range(len(style_layers)):
      layer_idx = style_layers[i]
      target_gram = style_targets[i]              # (N, R, C, C)
      mask = content_masks[layer_idx]             # (N, R, H, W)
      feat = feats[layer_idx]       # (N, 1, C, H, W) → make room for R dim

      # Broadcast features across R regions
      feat = feat.expand(-1, mask.shape[1], -1, -1, -1)  # (N, R, C, H, W)

      current_gram = guided_gram_matrix(feat, mask)      # (N, R, C, C)

      # Compute MSE loss over all N and R
      layer_loss = torch.sum((current_gram - target_gram) ** 2)
