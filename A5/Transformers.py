...
def prepocess_input_sequence(
    ...
    for token in tokens:
        if token in spc_tokens:
            ...
            # Split numbers into individual digits and map them separately
            out.extend(token_dict[digit] for digit in token)
        else:
            # Map other words normally
            out.append(token_dict[token])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K, M = query.shape
    out = torch.zeros_like(query, dtype=query.dtype, device=query.device)  # 初始化输出矩阵 (K, M)
    sqrt_M = M ** 0.5  # 计算缩放因子

    for i in range(K):
        # 计算 query[i] 和所有 key 的点积，存入 scores
        scores = torch.zeros(K)
        for j in range(K):
            scores[j] = torch.dot(query[i], key[j]) / sqrt_M  # 计算缩放后的分数
        
        # 对 scores 应用 softmax
        attention_weights = torch.softmax(scores, dim=0)

        # 计算加权求和，得到新的输出向量
        out[i] = torch.sum(attention_weights[:, None] * value, dim=0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out = torch.zeros_like(query, dtype=query.dtype, device=query.device)  # 初始化输出张量 (N, K, M)
    sqrt_M = M ** 0.5  # 计算缩放因子

    for n in range(N):  # 遍历 batch 中的每个样本
        scores = torch.zeros((K, K))  # 存储 K 个 query 对所有 key 的打分

        for i in range(K):  # 遍历每个 query[i]
            for j in range(K):  # 遍历每个 key[j]
                scores[i, j] = torch.dot(query[n, i], key[n, j]) / sqrt_M  # 计算缩放后的分数

        # 对 scores 计算 softmax，获取 attention 权重
        attention_weights = torch.softmax(scores, dim=1)  # softmax 归一化到 (K, K)

        # 计算加权求和，得到最终输出
        out[n] = torch.matmul(attention_weights, value[n])  # 矩阵乘法计算加权 value
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    import math
    sqrtM = math.sqrt(M)
    weights = query.bmm(key.transpose(1,2)) / sqrtM
    if mask is not None:
        ##########################################################################
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        weights = weights.masked_fill(mask, value=torch.tensor(-1e9))
    # Replace "pass" statement with your code
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        # Step 2: 调用 Self-Attention 计算加权和
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        heads_outputs = [head(query, key, value, mask) for head in self.heads]  # List of (N, K, dim_v)
        y = self.linear_attn(torch.cat(heads_outputs, dim=-1))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        out1 = self.multihead_attn(x,x,x)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.zeros((1, K, M), dtype = torch.float32)
    for i in range(M):
        for j in range(K):
            y[0, j, i] = j / K
    ##############################################################################
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    for i in range(K):
        for j in range(M):
            if j % 2 == 0:
                a = math.floor(j/M)
                y[0,i,j] = math.sin(i/math.pow(10000,a))
            else:
                a = math.floor((j-1)/M)
                y[0,i,j] = math.cos(i/math.pow(10000,a))
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        dec_out_tmp = self.decoder(a_emb_inp, enc_out, mask)
        dec_out = dec_out_tmp.reshape(-1, dec_out_tmp.shape[2])
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
