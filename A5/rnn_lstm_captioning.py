...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # Compute the affine transformation and apply tanh activation
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # Replace "pass" statement with your code
    N, T, D = x.shape
    H = h0.shape[1]  # Extract hidden size H
    # Initialize output hidden state tensor
    h = torch.zeros(N, T, H, dtype=x.dtype, device=x.device)
    # Initialize list for cache storage (needed for backpropagation)
    cache = []
    # Set initial hidden state
    prev_h = h0
    # Iterate through time steps
    for t in range(T):
        # Forward step for the current timestep
...
def rnn_backward(dh, cache):
    ...
    # Replace "pass" statement with your code
    N, T, H = dh.shape
    D = cache[0][0].shape[1]  # Extract input dimension D from cache
    ...
    dprev_h = torch.zeros(N, H, dtype=dh.dtype, device=dh.device)  # Gradient for previous hidden state

    # Backward loop over time steps (reverse order)
    for t in reversed(range(T)):
        dnext_h = dh[:, t, :] + dprev_h  # Sum gradients from loss and future timestep
        dx[:, t, :], dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h, cache[t])

        dWx += dWx_t  # Accumulate gradients for Wx
        dWh += dWh_t  # Accumulate gradients for Wh
        db += db_t  # Accumulate gradients for bias

    dh0 = dprev_h  # Gradient of the initial hidden state
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class WordEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        out = self.W_embed[x]  # Index into embedding matrix using x
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return out
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        N, H = prev_h.shape
        # Combine input and previous hidden state using learned weights
        Wx, Wh, b = self.Wx, self.Wh, self.b  # Assume weights are stored in `self`
        # Compute LSTM gate activations: (N, 4H)
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        # Initialize cell state c0
        c0 = torch.zeros_like(h0)

        # Initialize output tensor for hidden states (N, T, H)
        ...
        for t in range(T):
            ...
            hn[:, t, :] = prev_h  # Store hidden state for each timestep
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    A_flat = A.view(N, H, -1)  # Shape: (N, H, 16)

    # Compute attention scores (scaled dot-product)
    attn_scores = torch.bmm(prev_h.unsqueeze(1), A_flat)  # Shape: (N, 1, 16)
    attn_scores = attn_scores.squeeze(1) / (H ** 0.5)  # Scale by sqrt(H), shape: (N, 16)

    # Compute attention weights (Softmax over last dimension)
    attn_weights = F.softmax(attn_scores, dim=1)  # Shape: (N, 16)

    # Compute attention output
    attn = torch.bmm(A_flat, attn_weights.unsqueeze(2)).squeeze(2)  # Shape: (N, H)
