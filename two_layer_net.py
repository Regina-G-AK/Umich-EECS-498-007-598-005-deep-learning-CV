...
def nn_forward_backward(
    ...
    dh1 = dscores @ W2.T
    dh1[h1 <= 0] = 0  # Backpropagate through ReLU
...
def nn_predict(
    ...
    # Replace "pass" statement with your code
    # 前向传播：隐藏层计算
    Z1 = X @ params["W1"] + params["b1"]  # 第一层线性变换
    A1 = torch.relu(Z1)                  # 激活函数（ReLU）
    # 前向传播：输出层计算
    Z2 = A1 @ params["W2"] + params["b2"]  # 第二层线性变换
    # 预测：选择每行最大分数对应的类别
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-1, 2e-1, 5e-1, 7e-1, 8e-1, 9e-1, 1e0]
    hidden_sizes = [128, 256]
    regularization_strengths = [1e-7, 1e-6, 1e-5, 5e-5, 1e-4]
            ...
            for reg in reg_strengths:
                for lr_decay in lr_decays:
                    ...
                    stats = net.train(
                        X_train, y_train,
                        X_val, y_val,
                        num_iters=2000,  # 固定训练迭代次数
                        batch_size=200,  # 固定批量大小
                        learning_rate=lr,
